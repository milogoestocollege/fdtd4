...
static double *ezOldLeft1, *ezOldLeft2, 
            *ezOldRight1, *ezOldRight2;
static double *abcCoefLeft, *abcCoefRight;


// initialization for 1st-order abc
...
void abcInit(Grid *g){
    ...
    // calc coefficient on left end of grid
    ...
    // calc coef on right end of grid
...
void abc(Grid *g){
    ...
    if (!initDone) {
        fprintf(stderr, "abc: abcInit must be called before abc.\n");
        exit(-1);
    }
